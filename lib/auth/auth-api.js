...
class AuthApi {
    constructor(config, db, router = (0, express_1.Router)()) {
        config = Object.assign({ requireScopes: true, allowHandshakes: true, allowMasterKeys: true, handshakeExpTime: 300000 }, config);
        this._router = router;
        const validateSession = (0, auth_middleware_1.validateUserSession)(db);
        router.get('/type', (_, res) => res.send(''));
        // auth
        const authRouter = (0, express_1.Router)();
        authRouter.post('/login', (0, express_1.json)(), (0, middleware_1.wrapAsync)(async (req, res) => {
            ...
            const pass = await (0, auth_util_1.hash)(user.salt, req.body.password);
            if (user.pass !== pass)
                throw new errors_1.AuthError('Username / password mismatch.');
            res.send(await db.addSession(user.id, ['!user', '/']));
        }), middleware_1.handleValidationError);
        authRouter.post('/register', (0, express_1.json)(), (0, middleware_1.wrapAsync)(async (req, res) => {
            ...
            const user = {
                ...
                salt,
                pass: await (0, auth_util_1.hash)(salt, req.body.password)
            };
            await db.addUser(user);
            res.sendStatus(204);
        }));
        authRouter.post('/change-pass', validateSession, (0, express_1.json)(), (0, middleware_1.wrapAsync)(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
            if (!req.body.password || !req.body.newpass)
                throw new errors_1.MalformedError('Body must have a password, and a newpass.');
            if (await (0, auth_util_1.hash)(req.user.salt, req.body.password) !== req.user.pass)
                throw new errors_1.NotAllowedError('Password mismatch.');
            const salt = (0, crypto_1.randomBytes)(128).toString('hex');
            const pass = await (0, auth_util_1.hash)(salt, req.body.newpass);
        ...
        authRouter.get('/sessions', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
        ...
        authRouter.delete('/sessions/:id', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
            const sess = await db.getSession(req.params.id);
            if (!sess || sess.user !== req.user.id)
                throw new errors_1.NotFoundError('No session found with the id ' + req.params.id + ' for user ' + req.user.username + '!');
            await db.delSession(sess.id);
            res.sendStatus(204);
        }));
        authRouter.delete('/sessions', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
        ...
        authRouter.post('/logout', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            await db.delSession(req.session.id);
            res.sendStatus(204);
        }));
        authRouter.get('/refresh', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            const sess = await db.addSession(req.user.id, req.session.scopes);
            await db.delSession(req.session.id);
            res.json(sess);
        }));
        if (config.allowHandshakes) {
            const handshakeRouter = (0, express_1.Router)();
            handshakeRouter.get('/start', (0, middleware_1.wrapAsync)(async (req, res) => {
                ...
                if (req.query.scopes) {
                    info.scopes = this.validateScopes(req.query.scopes);
                }
                else
                    delete info.scopes;
            ...
            handshakeRouter.use('/:id', validateSession, (0, middleware_1.wrapAsync)(async (req, res, next) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
            ...
            handshakeRouter.get('/:id', (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
                res.json({
            ...
            handshakeRouter.get('/:id/approve', (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
                let code;
                do {
                    code = (0, crypto_1.randomBytes)(24).toString('hex');
            ...
            handshakeRouter.get('/:id/cancel', (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
                await db.delHandshake(req.handshake.id);
            ...
            authRouter.post('/session', (0, middleware_1.wrapAsync)(async (req, res) => {
        ...
        if (config.allowMasterKeys) {
            const masterKeyRouter = (0, express_1.Router)();
            masterKeyRouter.get('', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
                res.json(await db.getMasterKeysForUser(req.user.id));
            }));
            masterKeyRouter.post('', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
            ...
            masterKeyRouter.put('/:id', validateSession, (0, express_1.json)(), (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
            ...
            masterKeyRouter.delete('/:id', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
                if (!req.session.scopes.includes('!user'))
                    throw new errors_1.NotAllowedError('Must be a user!');
                const masterkey = await db.getMasterKey(req.params.id);
                if (!masterkey || masterkey.user !== req.user.id)
            ...
            authRouter.use('/master-key', masterKeyRouter, (0, middleware_1.handleError)('auth-master-key'));
            authRouter.post('/generate-session', (0, middleware_1.wrapAsync)(async (req, res) => {
                ...
                let scopes;
                if (req.query.scopes)
                    scopes = this.validateScopes(req.query.scopes);
        ...
        router.use('/auth', authRouter, (0, middleware_1.handleError)('auth'));
        router.get('/self', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            res.json({ id: req.user.id, username: req.user.username });
        }), (0, middleware_1.handleError)('get-self'));
        router.delete('/self', validateSession, (0, middleware_1.wrapAsync)(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
            if (!req.query.pass || typeof req.query.pass !== 'string')
                throw new errors_1.MalformedError('?pass=.. required!');
            if (await (0, auth_util_1.hash)(req.user.salt, req.query.pass) !== req.user.pass)
                throw new errors_1.NotAllowedError('Password does not match!');
            await db.delUser(req.user.id);
            res.sendStatus(204);
        }), (0, middleware_1.handleError)('delete-self'));
    }
    get router() { return this._router; }
    validateScopes(scopes, requireScopes = false) {
        let ret;
        try {
            ...
            throw new errors_1.MalformedError('Could not parse scopes query; should be a JSON array of paths that start with "/".');
        }
        return ret;
    }
}
