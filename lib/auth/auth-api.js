...
class AuthApi {
    constructor(config, db, router = express_1.Router()) {
        ...
        authRouter.post('/change-pass', validateSession, express_1.json(), middleware_1.wrapAsync(async (req, res) => {
            ...
            const salt = crypto_1.randomBytes(128).toString('hex');
            const pass = await auth_util_1.hash(salt, req.body.newpass);
        ...
        authRouter.get('/sessions', validateSession, middleware_1.wrapAsync(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
        ...
        authRouter.delete('/sessions/:id', validateSession, middleware_1.wrapAsync(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
        ...
        authRouter.delete('/sessions', validateSession, middleware_1.wrapAsync(async (req, res) => {
            if (!req.session.scopes.includes('!user'))
                throw new errors_1.NotAllowedError('Must be a user!');
